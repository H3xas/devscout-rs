...
| r | src/Domain/Registry.cs | A same-id, two-arity interface pair (`IDictionary`/`IDictionary<T>`); a receiver written with an arity neither sibling has (`IDictionary<string, string>`, matching the BCL's own two-argument `IDictionary`) must stay external rather than fall back to the one-argument sibling, contrasted with an exact one-argument receiver that still binds precisely. |
