...
| `src/Api/Consumers/DeliveryScheduledConsumer.cs` | consume fact via a base-class consumer (`BaseConsumer<T>`); ctor-assigned fields with a null-guard; publish with an explicit type argument inside a private helper method; method_call for a call on each of its two ctor-injected fields (`_repository.Find`, and `_bus.Publish` -- a directly assigned, non-null-guarded field still counts) |
...
| `src/Api/Controllers/ParcelsController.cs` | route facts: class-level route + bare verb, `Route` combined with two bare verbs, `[action]` token with no verb, and a private helper that yields only a method_span; an inline message publish; method_call for `_repository.Find` and `_bus.Publish` on ctor-injected fields, plus a second `_repository.Find` from a private expression-bodied helper (`HasRecord`), proving method_call is not scoped to block bodies only |
| `src/Api/Handlers/GetParcelHandler.cs` | di_binding via `IRequestHandler<TRequest, TResponse>`; a request/response record pair; method_call for `_repository.Find` on a ctor-injected field |
