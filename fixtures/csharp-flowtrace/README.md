...
| `src/Api/Controllers/ParcelsController.cs` | route facts: class-level route + bare verb, `Route` combined with two bare verbs, `[action]` token with no verb, and a private helper that yields only a method_span; an inline message publish; method_call for `_repository.Find` and `_bus.Publish` on ctor-injected fields |
