...
fn map_rerun_reuses_fragments_and_a_missing_cache_re_extracts_with_spans() {
    ...
    let v19 = graph_dir.join("fragments-v19.json");
    assert!(v19.exists(), "the current cache generation is on disk");

    // Simulate the pre-bump world: only a superseded pair present. BOTH v19 files
    ...
    // writes the v19 pair again, and deletes the superseded generation.
    fs::write(graph_dir.join("fragments-v16.json"), b"{}").unwrap();
    fs::remove_file(&v19).unwrap();
    fs::remove_file(graph_dir.join("fragments-index-v19.json")).unwrap();

    let rebuild = fx.run(&["map", "."]);
    assert!(rebuild.status.success(), "{rebuild:?}");
    assert!(v19.exists(), "re-extraction rewrote the current generation");
    assert!(
